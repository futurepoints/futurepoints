...
{
    resize(850, 550);
    setWindowTitle(tr("FuturePoints") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a FuturePoints address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About FuturePoints"), this);
    aboutAction->setToolTip(tr("Show information about FuturePoints"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for FuturePoints"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("FuturePoints client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("FuturePoints client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to FuturePoints network.", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid FuturePoints address or malformed URI parameters."));
    }
